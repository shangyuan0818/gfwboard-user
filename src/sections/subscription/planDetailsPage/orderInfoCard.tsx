    ...
    <MainCard title={t("subscription.order-info-card.title")}>
      <Stack spacing={2} divider={<Divider />}>
        ...
        {couponCode && (
          <Stack direction={"row"} justifyContent={"space-between"} spacing={1}>
            <Typography variant={"body1"}>
              {t("subscription.order-info-card.coupon", {
        ...
        <Stack direction={"column"} spacing={2}>
          <Typography variant={"subtitle1"}>{t("subscription.order-info-card.total")}</Typography>
          ...
          <Button fullWidth variant={"contained"} color={"primary"} onClick={handleClick} disabled={isSubmitting}>
            {t("subscription.order-info-card.button")}
          </Button>
        </Stack>
      </Stack>
    </MainCard>
